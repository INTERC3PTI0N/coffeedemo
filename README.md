...
**The dive.** What stands in for the ridges is the bean field itself, run as a
fly-through. A corridor of beans stretches back into the cloud bank; each bean
holds one fixed lateral offset for its whole run and simply closes on the
camera, so the way it spreads across the frame is honest perspective rather
than a pose. That is what makes it read as travel instead of as a swirl, and it
...
with golden-ratio steps looks decorrelated and is not, and forty-four beans
line up into a pair of visible spiral arms. The near end stops at z 8, clear of
the dollied camera at 9.8, or the last stretch of the corridor ends up behind
the lens with beans blinking out against the near plane. And the wrap: a bean
that reaches the lens has to start again at the far end, and the field eases
toward its targets rather than snapping, so that jump would drag it backwards
through the corridor in plain sight. Both ends of the run fade, and the frame
